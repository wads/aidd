...
stacked PR の機構は `gh stack`（GitHub ネイティブ機能）に委ね、**操作手順は公式の `gh-stack` skill を単一の正とする**。生の git コマンドによる積み替え・base 付け替えは行わない（例外はスタックの再構成。公式手順が生の git を使うため）。
...
- **非対話での挿入・分割・並べ替え・改名・削除は存在しない**（`gh stack modify` は TUI 専用で、エージェントが起動してはならず、メタデータを変えても git の祖先関係は変わらない）。回避策は**コミットの並びを git で組み替えてから** `unstack` → `init` でスタックを作り直す
