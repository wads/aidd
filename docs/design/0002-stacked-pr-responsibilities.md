...
- Status: Accepted（2026-09-04 人間承認）
- Date: 2026-09-04
...
|---|---|---|
| **stacked-pr（新設）** | 手順の単一の正: PR 作成（base 指定・ステップ 1 は既定ブランチ）、積み替え（`rebase --update-refs`）、base 付け替え、squash マージ後の追随、diff 不変判定(`range-diff`)と再ゲート要否・チェック済み失効の判定、コンフリクト境界（自明= AI / 判断あり= 人間）、force-with-lease の例外規定、計画変更時のスタック構成更新、環境要件（git 2.38+） | AC-2, 5, 6, 8, 9 / QC-1, 3 |
...
5. **最終 PR**: マージ前に Issue 全 AC⇄テストの横断確認。漏れはマージ済み分を含め追加修正 PR（前方修正）
