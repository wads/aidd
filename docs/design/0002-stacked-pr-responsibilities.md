...
- Updated: 2026-09-06（ADR-0007 反映）
- Date: 2026-09-04
...
| **stacked-pr（aidd）** | プロセスの規則: 適用条件、ステップ完了の定義と PR 作成タイミング、先行着手と中断、積み替え後の失効と再ゲート（ADR-0006）、コンフリクト境界（自明 = AI / 判断あり = 人間）、チェック依頼の形、環境要件の定義（gh 2.90+・拡張・公式 skill・repo での有効化。**要件を満たすかの判定と、満たさない場合の人間確認は `implementation-plan` が P5 で行う**） | AC-2, 3, 5, 6, 8, 9 / QC-1, 3 |
...
| critical-gate | 適用単位 = PR の差分ごと。積み替え・base 付け替え（自動再ターゲットを含む）をした PR は常に再ゲート（免除の判定は持たない。ADR-0006） | AC-4, 9 |
...
- **最終 PR で追加実施**: context-snapshot の最終版（各 PR の受入れ判断でも同じ URL を更新する。ADR-0009）、短期コンテキストの昇格判定、全 AC⇄テスト対応表の集約と横断確認（AC-10）

## フロー（どのフェーズでどの操作が走るか)

コマンドは書かない（ADR-0007 の委譲方針。操作手順は公式 `gh-stack` skill が正）。

1. **P5**: 環境要件の確認 → 計画承認 → スタック構成確定 → 計画全体を Issue コメント → スタックを作り最初の draft PR を出す
2. **P6（ステップごとの繰り返し）**: 層を積む → 実装 → ステップ完了 → PR を出す → PR 差分の critical-gate → 人間チェック依頼 → 待たずに次ステップへ着手
3. **随時（下位に指摘対応）**: 該当の層で修正 → 上位を積み替え → push → 積み替えた全 PR を失効させ（draft へ戻す）、そのステップの検証を再実行（再ゲートは次にチェックへ出す直前）
4. **随時（チェック済みのマージ）**: 最下段の未マージ PR を明示指定して 1 本ずつマージ → 着地確認 → 上位を同期（積み替え）→ 上位は base 再ターゲットにより失効
5. **最終 PR**: 全 PR の AC⇄テスト対応表を集約し、マージ前に Issue 全 AC⇄テストの横断確認。漏れはマージ済み分を含め追加修正 PR（前方修正）。context-snapshot の最終版・昇格判定もここで実施（snapshot は各 PR の受入れ判断でも同じ URL を更新）
