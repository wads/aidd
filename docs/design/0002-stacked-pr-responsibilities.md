...
4. **随時（チェック済み最下段のマージ)**: マージ → 既定ブランチへの着地確認 → ブランチ削除 → 次 PR の base を既定ブランチへ付け替え（squash 時はマージ済み差分を rebase で落とす）→ ready 化 → 3. と同じ判定
