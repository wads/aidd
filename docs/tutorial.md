...
- 着手したい GitHub Issueがあること（なければ `/product-intent` で作る）

## Step 0: Issue を作る — `/product-intent`

開発はIssue から始まります。`/product-intent` が要求をタイプ別テンプレートで構造化し、`type-*` ラベルつきの GitHub Issueを作ります。

```
> /product-intent 検索画面で表示が崩れているのを直したい
```

タイプ（feature / bugfix / hotfix / refactoring / chore / spike）はここで確定します。bugfix や chore などエンジニア起点のIssue は軽量テンプレートで数分で作れます。
...
`/dev` はIssue の `type-*` ラベルからルート（実施フェーズ構成）を提示します。あなたがルートを承認すると、フェーズが順に進みます。
...
- 確定版を承認する（Issue コメントに残ります）

## Step 3: TDD で実装（P6 実装）
...
AAR（意図と結果の差分）で振り返ります。まず「想定と違ったのはどこか」を聞かれるので、そこから対話が始まります。フェーズごとの意図と結果を突き合わせ、ズレの原因を掘り、記録されなかった判断がないかを点検します。合意した変更がこの playbook への PR になります。
...
skill 本文の source of truth は `.claude/skills/<name>/SKILL.md` です。
Claude Code がネイティブ skill として認識するため、`/dev` のようにそのまま起動できます（別途の wrapper は不要）。
