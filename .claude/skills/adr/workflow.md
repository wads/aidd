...
何を決めるのか、なぜ今決める必要があるのかを 1〜2 行で確認する。判断に選択の余地がない（既知のパターンの適用のみ）なら ADR は作らない。ただし P4 の構造基準（`design-docs/workflow.md` §1）に当たる変更では、作らない旨と理由を人間に提示して確定する（「設計書なし」と同じ問いにまとめてよい）。

- **同 topic の既存 ADR を読む**: ADR ディレクトリの `INDEX.md` から該当 topic の有効 ADR を引き、要旨で絞ってから読み、この判断がそれらと「置き換え / 補足 / 無関係」のどれに当たるかを frontmatter に書く（置き換え → `supersedes`、補足・部分修正 → `amends`、それ以外＝無関係または依存して併存 → `considered`）。同 topic で自分より若い番号の有効 ADR が 3 つのどれにも無ければスクリプトが error にする。既存の決定を変えるのに置き換え・補足を宣言しない ADR は、衝突を後から検知できない最大の経路である
- 語彙表（`{records_root}/domain-terms.md`。ハブは `system/domain-terms.md`）に合う topic が無ければ、この ADR と同じコミットで語彙表へ追加する。意味欄は既存 topic との境界が分かる書き方にする（新 topic だけを名乗る ADR は網羅検査の対象外になるため、レンズが語彙表を見て判断する）。語彙表自体が無ければ `shared/templates/domain-terms-template.md` から作り、最初の ADR と同じコミットに含める
...
最終決定は人間が行う。提示には `context-snapshot` を生成・更新して添える（同じ URL。この時点では設計・実装の節は「未着手」でよい）。採用されなかった懸念・反対意見も「検討した選択肢」として記録してよい。
