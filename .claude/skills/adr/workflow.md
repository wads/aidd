...
何を決めるのか、なぜ今決める必要があるのかを 1〜2 行で確認する。判断に選択の余地がない（既知のパターンの適用のみ）なら ADR は作らない。ただし P4 の構造基準（`design-docs/workflow.md` §1）に当たる変更では、作らない旨と理由を人間に提示して確定する（P4 の設計承認の問いにまとめてよい）。
