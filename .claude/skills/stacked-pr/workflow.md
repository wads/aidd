...
- **ステップ分割**: 対象ブランチのコミットを 2 ブランチに分け（`git rebase -i` 相当の分割）、上位を積み替える。**分割後の後半ブランチで新しい PR を作り、直上の PR の base を後半ブランチへ付け替える**（元の PR は前半ブランチのまま残す）。元の PR は差分が必ず変わるため、§4 の判定を経て §5 の失効規則に従う
...
- 構成変更で差分が変わった PR は §4 の判定 → §5 の失効規則に従う（再ゲートが必要になりうる）
