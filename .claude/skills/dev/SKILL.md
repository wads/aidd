...
- フェーズの実施有無はルート（`routes.md`）が決め、成果物の深さは判断駆動の原則が決める（構造基準に当たれば設計書は必須。`design-docs/workflow.md` §1）
...
- 積み替え・マージの手順は `stacked-pr` を参照する（機構は GitHub ネイティブ機能 `gh stack` が持つ）。P6 中に計画が変わったときのスタック構成の更新は `stacked-pr/workflow.md` §7（最上段への追加は通常フロー。途中への挿入・分割・並べ替え・改名・削除には非対話の手段が無く、コミットの並びを組み替えてスタックを作り直す）
