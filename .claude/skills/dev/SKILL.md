...
- 積み替え・マージの手順は `stacked-pr` を参照する（機構は GitHub ネイティブ機能 `gh stack` が持つ）。P6 中に計画が変わったときのスタック構成の更新は `stacked-pr/workflow.md` §7（`gh stack modify` は対話型のため、自動進行中は人間に実行を依頼する）
