...
| P4 設計（design-docs） | 判断駆動（構造基準に当たれば必須） | − | 判断駆動（構造基準に当たれば必須） | − | − |
...
- 「判断駆動」= フェーズには入るが、成果物（ADR・設計書）は書くべき判断が生じたときだけ作る。P4 の「構造基準に当たれば必須」= テスト以外のコードを新設する、または既存の責務分割・依存方向を変える変更では設計判断が生じたとみなし設計書を必ず作る（規模軸ではない）。それ以外で作らない場合も「設計書なし」を人間に提示して確定し、構造基準に当たる変更では「ADR なし」も同様に確定する（`design-docs/workflow.md` §1）
