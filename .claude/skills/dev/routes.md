...
| P4 設計（design-docs） | 必須（規模基準）/ 判断駆動 | − | 必須（規模基準）/ 判断駆動 | − | − |
...
- 「判断駆動」= フェーズには入るが、成果物（ADR・設計書）は書くべき判断が生じたときだけ作る。P4 の「必須（規模基準）」= テスト以外のコードを新設する、または既存の責務分割・依存方向を変える変更では設計書を必ず作る。それ以外で作らない場合も「設計書なし」を人間に提示して確定する（`design-docs/workflow.md` §1）
