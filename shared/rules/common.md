...
- TDD を優先する
- 変更を小さく刻む
- 途中の発見（アイデア・改善候補・スコープ外化した要求）は、まず安い場所に捕捉する（親 Issue／PR のコメント、`roadmap` の候補欄、要求の「対象範囲＞対象外」）。**Issue 化はコミットメント**であり、着手を決めたときだけ行う（数分で直せる負債はその場で直す）。捕捉した発見のトリアージ（昇格 / その場修正 / 候補保持 / 破棄）は P7・P8 でまとめて行い、**フロー中に未着手の個別 Issue を量産しない**（未検証のアイデアを一級 Issue として滞留させない）
